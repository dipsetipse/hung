 ...
 * @author cemNguyeH
 */
public class GravesitePreplacedSectionDisxls extends HttpServlet 
{
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
  {
        ...
        String distID = req.getParameter("dist_id");
        ...
        if (!(Utility.setRole(conn, "nca_sic"))) {
           ...
           String qry = "select nme from district where id = ?";
        ...
        try {
                ...
                pstmt = conn.prepareStatement(qry);
                pstmt.setString(1, distID); //if you want to add parameter in the query
                pstmt.setString(2, startDT); 
                rs = pstmt.executeQuery();

                //instruct the browser to send output as Excel
                res.setContentType(CONTENT_TYPE);
                res.setHeader("Content-Disposition","attachment; filename=GPS_dis" + distID + " " + startDT + ".xls");   // filename
                ServletOutputStream out= res.getOutputStream();

                //header in Excel
                out.println ("History-Status Report \n" + 
                             distID + " " + distNme + ", " + startDT + "\n" );
